...
if(kind==='plan'){
 ...
 const lay=p.layout||{}, chamberL=Number(lay.pumpLength||12)*k,chamberB=Number(lay.pumpWidth||8)*k, side=lay.pumpSide||'East';
 ...
 const edge={East:[cx+g.L*k/2,cy],West:[cx-g.L*k/2,cy],North:[cx,cy-g.B*k/2],South:[cx,cy+g.B*k/2]}[side];
 const horizontal=side==='East'||side==='West', sign=(side==='East'||side==='South')?1:-1;
 const px=edge[0]+(horizontal?sign*(chamberL/2+18):0),py=edge[1]+(!horizontal?sign*(chamberB/2+18):0);
 const rw=horizontal?chamberL:chamberB,rh=horizontal?chamberB:chamberL;
 s+=`<rect x="${px-rw/2}" y="${py-rh/2}" width="${rw}" height="${rh}" style="fill:#d7e8ef;stroke-width:2"/>`+tx(px,py+4,'PUMP')+tx(px,py+20,'CHAMBER');
 const topY=cy-g.wb*k*.27,bottomY=cy+g.wb*k*.27, pipeX=edge[0],pipeY=edge[1];
 s+=ln(px,py,pipeX,pipeY,'')+ln(pipeX,pipeY,horizontal?cx:cx,horizontal?topY:cy,'water')+ln(pipeX,pipeY,horizontal?cx:cx,horizontal?bottomY:cy,'bottom');
 s+=`<ellipse cx="${cx}" cy="${topY}" rx="${Math.max(20,g.wl*k*.25)}" ry="7" style="fill:#bfe6f2;stroke:#197fa4"/><ellipse cx="${cx}" cy="${bottomY}" rx="${Math.max(20,g.wl*k*.25)}" ry="7" style="fill:#f6ddb0;stroke:#956e49"/>`;
 s+=tx(cx,topY-12,'TOP DIFFUSER — charge')+tx(cx,bottomY+23,'BOTTOM DIFFUSER — discharge');
 const wells=Math.max(0,Math.round(Number(lay.drainageWells||0))); for(let i=0;i<wells;i++){const a=(i+.5)*Math.PI*2/wells,wx=cx+Math.cos(a)*(g.L*k/2+perm*k/2),wy=cy+Math.sin(a)*(g.B*k/2+perm*k/2);s+=`<circle cx="${wx}" cy="${wy}" r="5" style="fill:#f6ce5a"/>`+tx(wx,wy-9,'D');}
 s+=tx(800,625,'CONCEPT COMPONENTS',17)+tx(800,652,'Pump chamber: '+n(Number(lay.pumpLength||0))+' × '+n(Number(lay.pumpWidth||0))+' m')+tx(800,677,'Design flow: '+n(Number(lay.designFlowM3h||0))+' m³/h')+tx(800,702,'Diffuser area at '+n(Number(lay.diffuserVelocityMps||0),3)+' m/s: '+n(Number(lay.diffuserAreaM2||0))+' m²');
