...
def plan_svg(inputs: dict, m: dict, layers: dict | None = None) -> str:
    layers = {"lid": True, "hydraulics": True, "drainage": True, "dimensions": True, **(layers or {})}
    ...
    pipe = f'<path d="M {px:.1f} {cy:.1f} L {cx+m["wl"]*scale/2:.1f} {cy:.1f}" stroke="#d32f2f" stroke-width="5" fill="none"/><path d="M {cx+m["wl"]*scale/2:.1f} {cy:.1f} L {cx:.1f} {cy:.1f}" stroke="#1565c0" stroke-width="4" stroke-dasharray="7 4" fill="none"/>' if layers['hydraulics'] else ''
    ballast = ''.join(f'<line x1="{cx-m["wl"]*scale/2+i*m["wl"]*scale/13:.1f}" y1="{cy-m["wb"]*scale/2:.1f}" x2="{cx-m["wl"]*scale/2+i*m["wl"]*scale/13:.1f}" y2="{cy+m["wb"]*scale/2:.1f}" stroke="#64748b" stroke-width="1.2"/>' for i in range(1,13)) if layers['lid'] else ''
    ...
    dimensions = f'''<path class="thin" d="M {cx-m['L']*scale/2} {cy+m['B']*scale/2+35} H {cx+m['L']*scale/2}"/>
    ...
    <text class="dim" x="{cx-m['L']*scale/2-43}" y="{cy}" text-anchor="middle" transform="rotate(-90 {cx-m['L']*scale/2-43} {cy})">{_f(m['B'])} m rim width</text>''' if layers['dimensions'] else ''
    return _svg(f'''
    ...
    {envelope}{permanent}{rim}{water}{bottom}{ballast}{pipe}{pump if layers['hydraulics'] else ''}{wells if layers['drainage'] else ''}
...
def section_svg(inputs: dict, m: dict, layers: dict | None = None) -> str:
    layers = {"liner": True, "lid": True, "hydraulics": True, "drainage": True, "dimensions": True, **(layers or {})}
    ...
    def one_section(x0, top, water, bottom, title):
        ...
        liner = f'<path d="M {left} {rim_y} L {bot_left} {base_y} L {bot_right} {base_y} L {right} {rim_y}" fill="none" stroke="{LINER}" stroke-width="3"/><text class="sub" x="{left+25}" y="{rim_y+115}">protective geotextile + welded liner</text>' if layers['liner'] else ''
        lid = f'<path d="M {wl_left} {water_y} L {wl_right} {water_y}" stroke="#475569" stroke-width="10"/><path d="M {wl_left} {water_y-7} L {wl_right} {water_y-7}" stroke="#b0bec5" stroke-width="4" stroke-dasharray="8 3"/><text class="sub" x="{x0+top*s/2:.1f}" y="{water_y-18:.1f}" text-anchor="middle">floating membrane + insulated lid / ballast pipes</text>' if layers['lid'] else ''
        hydraulic = f'<path d="M {bot_left+15} {base_y-25} H {bot_right-15}" stroke="#1565c0" stroke-width="5"/><circle cx="{bot_right-25}" cy="{base_y-25}" r="7" fill="#1565c0"/><path d="M {wl_right-30} {water_y+23} H {right+35}" stroke="#d32f2f" stroke-width="6"/><text class="sub" x="{(bot_left+bot_right)/2:.1f}" y="{base_y-35}" text-anchor="middle">bottom diffuser / return</text><text class="sub" x="{right+40}" y="{water_y+27}">top diffuser / supply</text>' if layers['hydraulics'] else ''
        drainage = f'<path d="M {bot_left} {base_y+7} H {bot_left+35}" stroke="#37474f" stroke-width="4"/><circle cx="{bot_left+38}" cy="{base_y+7}" r="6" fill="#37474f"/><text class="sub" x="{bot_left+48}" y="{base_y+11}">drainage sump</text>' if layers['drainage'] else ''
        dims = f'<path d="M {x0-22} {rim_y} V {base_y}" class="thin"/><text class="dim" x="{x0-30}" y="{(rim_y+base_y)/2:.1f}" transform="rotate(-90 {x0-30} {(rim_y+base_y)/2:.1f})" text-anchor="middle">{_f(m['H'])} m pit depth</text><path d="M {bot_left} {base_y+28} H {bot_right}" class="thin"/><text class="dim" x="{(bot_left+bot_right)/2:.1f}" y="{base_y+45}" text-anchor="middle">{_f(bottom)} m bottom</text><path d="M {left} {rim_y-25} H {right}" class="thin"/><text class="dim" x="{(left+right)/2:.1f}" y="{rim_y-32}" text-anchor="middle">{_f(top)} m rim</text>' if layers['dimensions'] else ''
...
def isometric_svg(inputs: dict, m: dict, layers: dict | None = None) -> str:
    """Construction-aware axonometric concept, deliberately not a terrain model."""
    layers = {"liner": True, "lid": True, "hydraulics": True, "drainage": True, "dimensions": True, **(layers or {})}
    ...
    liner = ''.join(f'<path d="M {rim[i][0]:.1f} {rim[i][1]:.1f} L {bottom[i][0]:.1f} {bottom[i][1]:.1f}" stroke="{LINER}" stroke-width="4"/>' for i in range(4)) if layers['liner'] else ''
    lid = poly(water, '#64748b', '#334155') if layers['lid'] else ''
    ballast = ''.join(f'<line x1="{water[0][0]+i*(water[1][0]-water[0][0])/11:.1f}" y1="{water[0][1]+i*(water[1][1]-water[0][1])/11:.1f}" x2="{water[3][0]+i*(water[2][0]-water[3][0])/11:.1f}" y2="{water[3][1]+i*(water[2][1]-water[3][1])/11:.1f}" stroke="#cbd5e1" stroke-width="1.5"/>' for i in range(1,11)) if layers['lid'] else ''
    pipes = f'<path d="M {water[1][0]:.1f} {water[1][1]:.1f} L {water[1][0]+110:.1f} {water[1][1]-35:.1f}" stroke="#d32f2f" stroke-width="6"/><path d="M {bottom[1][0]:.1f} {bottom[1][1]:.1f} L {bottom[1][0]+100:.1f} {bottom[1][1]-25:.1f}" stroke="#1565c0" stroke-width="6"/>' if layers['hydraulics'] else ''
    drainage = ''.join(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="7" fill="#37474f"/>' for x,y in bottom) if layers['drainage'] else ''
    ...
    {faces}{poly(bottom,'#84563d')}{poly(water,WATER,BLUE)}{lid}{ballast}{liner}{pipes}{drainage}
    <text class="label" x="{water[1][0]+120:.1f}" y="{water[1][1]-40:.1f}">energy hub / pump connection</text>
    ...
    <text class="sub" x="35" y="680">Engineering axonometric concept: earthwork pit, liner, floating membrane/lid, ballast-pipe pattern, supply/return pipe stubs and drainage points. It is not a survey-based terrain or construction model.</text>''')
